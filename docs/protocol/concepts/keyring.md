...
MY_VALIDATOR_ADDRESS=$(qubeticsd keys show dev0 -a)
```
...
:::warning
**NOTE**: Cosmos `secp256k1` keys are not supported on Qubetics due to compatibility issues with Ethereum transactions.
:::
...
yes $KEYPASSWD | qubeticsd keys add me
yes $KEYPASSWD | qubeticsd keys show me
# start qubeticsd with keyring-backend flag
qubeticsd --keyring-backend=file start
```
