...
```bash
qubeticsd start --rpc.laddr="tcp://127.0.0.1:26657"
```
...
looking for. For example, an Ethereum transaction on Qubetics (`MsgEthereumTx`) triggers an `event` of type `ethermint` and
...
Check the `Events` page to obtain the event list of each supported module on Qubetics.
:::
